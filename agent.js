...
• projects — массив проектов/продуктов. ПО УМОЛЧАНИЮ ПУСТОЙ []. НЕ угадывай и НЕ выводи проект «по смыслу» из контекста. Заполняй projects ТОЛЬКО если выполнено одно из условий:
  1) В тексте рядом стоит слово-маркёр: «проект», «продукт», «направление», «стрим», «инициатива», «программа», «эпик», «epic», «workstream», «команда» или их формы/синонимы в ИТ-контексте — тогда проект = название после этого слова (например: «проект Полис» → ["Полис"], «по стриму Биллинг» → ["Биллинг"]).
  2) В тексте есть слово, написанное с ЗАГЛАВНОЙ буквы НЕ в начале предложения, и это НЕ имя собственное (не имя/фамилия человека, не город, не компания) — обычно это внутреннее кодовое название продукта/проекта (например: «добавить факсимиле в Полис» → ["Полис"]). Слова в начале предложения и обычные нарицательные слова со строчной буквы (полис, биллинг, отчёт) НЕ считаются проектом.
Во всех остальных случаях projects = []. Не считай проектом отделы, технологии, общие термины и нарицательные существительные.
