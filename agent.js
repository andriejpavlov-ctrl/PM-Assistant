...
{"cards":[{"title":"...","description":"...","people":["..."],"projects":["..."],"priority":"P1|P2|P3|","deadline":null|"YYYY-MM-DD","tags":["..."]}]}
 ...
 * @param {object} [opts]  { forceSingle } — заставить вернуть ровно одну карточку
 * @returns {Promise<object[]>} массив карточек-черновиков (1+) с полями title, description, people, projects, priority, deadline, tags, rawText
 */
export async function processCapture(rawText, knownPeople = [], opts = {}) {
  ...
  const user = `Сегодняшняя дата: ${todayISO()}.${peopleHint}${singleRule}\n\nТекст пользователя:\n"""${rawText}"""\n\nРазбери и верни JSON.`;
